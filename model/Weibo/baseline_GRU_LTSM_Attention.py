...
class TDrumorGCN(th.nn.Module):
    ...
    def forward(self, data):
        ...
        root_extend = th.zeros(len(data.batch), x1.size(1)).to(x1.device)
        ...
        x = F.relu(x)
        root_extend = th.zeros(len(data.batch), x2.size(1)).to(x2.device)
        for num_batch in range(batch_size):
            index = (th.eq(data.batch, num_batch))
...
class Net(th.nn.Module):
    def __init__(self, in_feats, hid_feats, out_feats, num_heads=4):
        ...
        self.mha = MultiheadAttention(embed_dim=128, num_heads=num_heads, batch_first=True)
        
        # Fully connected layer after LSTM and MHA
    ...
    def forward(self, data):
        ...
        # Concatenate the outputs of TD and BU GCNs
        x = th.cat((BU_x, TD_x), 1).unsqueeze(1)  # Add an extra dimension for LSTM
        ...
        mha_out = mha_out[:, -1, :]  # Use the output from the last time step
