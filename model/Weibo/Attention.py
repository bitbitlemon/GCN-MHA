...
class Net(th.nn.Module):
    def __init__(self, in_feats, hid_feats, out_feats, num_heads=4):
        ...
        self.mha = MultiheadAttention(embed_dim=(out_feats + hid_feats) * 2, num_heads=num_heads, batch_first=True)
        
        # Fully connected layer after MHA
    ...
    def forward(self, data):
        ...
        x = th.cat((BU_x, TD_x), 1).unsqueeze(1)  # Add an extra dimension for MHA
        
        # Apply Multi-Head Attention
        mha_out, _ = self.mha(x, x, x)
        mha_out = mha_out.squeeze(1)  # Remove the extra dimension
