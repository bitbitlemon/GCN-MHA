...
class Net(th.nn.Module):
    def __init__(self, in_feats, hid_feats, out_feats):
        ...
        # Add a GRU layer
        self.gru = th.nn.GRU((out_feats + hid_feats) * 2, hid_feats, batch_first=True)
        
        # Fully connected layer after the GRU
        self.fc = th.nn.Linear(hid_feats, 2)

    def forward(self, data):
        TD_x = self.TDrumorGCN(data)
        BU_x = self.BUrumorGCN(data)
        x = th.cat((BU_x, TD_x), 1).unsqueeze(1)  # Add an extra dimension for GRU
        gru_out, _ = self.gru(x)  # GRU processing
        gru_out = gru_out.squeeze(1)  # Remove the extra dimension
        x = self.fc(gru_out)
